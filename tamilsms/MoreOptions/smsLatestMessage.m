...
#import "smsLatestMessage.h"
#import "smsRESTProxy.h"

@interface smsLatestMessage ()
{
...
@implementation smsLatestMessage
...
- (void)viewDidLoad {
    ...
    [self.navigationItem setTitle:@"Latest Message"];
    [self initializeData];
}
...
{
    _pageNo = 1;
    [[smsRESTProxy alloc] initDatawithAPIType:@"LATEST_TXT_MSGS" andInputParams:@{@"page":@(_pageNo)} andReturnMethod:^(NSData * p_txtmsgsData){
        ...
        NSLog(@"Latest MEssage list data %@", l_receiveddict);
...
{
    NSLog(@"the numberof rows are %lu",(unsigned long)_txtMessages.count);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    smsLatestMsgCell * l_cell = (smsLatestMsgCell*) [tableView dequeueReusableCellWithIdentifier:l_CellIdentifier];
    if (l_cell == nil)
    {
        l_cell = [[smsLatestMsgCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:l_CellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        //showlatestindtxtmsgs
        self.navigateParams = @{@"initialposn":@(indexPath.row),
                                @"allmessages":_txtMessages,
                                @"isonline":@(YES)};
        dispatch_async(dispatch_get_main_queue(), ^{
            [self performSegueWithIdentifier:@"showlatestindtxtmsgs" sender:self];
        });
    }
}
...
@implementation smsLatestMsgCell
